...
/*waarom geen while?*/ if(tijd<=e.getTime()){
                   tijd+=15;
               } 
            } 
        if(tijd<=e.getTime()){
/*waarom geen while?*/  tijd+=15;}
        System.out.println("vorige tijd"+ e.getTime());     
        e.setTime(tijd);
        return e;
    } 
        ...
        double vorigeScheduleTime=scheduleTimeUrgent;
                ...
                for(int j=0;j<urgentSlotsForToday.length;j++){
                    if(scheduleTimeUrgent==vorigeScheduleTime){
                        if(urgentSlotsOpenClosed[j].equals("Open")&&urgentSlotsForToday[j]>arrivalTime&&urgentSlotsForToday[j]>scheduleTimeUrgent){ //TIME AANGEPAST NAAR ARRIVALTIME, LAATSTE DEEL ZOU WEG MOETEN
